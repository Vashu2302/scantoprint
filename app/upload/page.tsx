...
function CustomerUploadContent() {
  ...
  useEffect(() => {
    async function loadShop() {
      if (shopId) {
        ...
        const { data } = await supabase.from('shops').select('*').order('created_at', { ascending: false }).limit(1).single();
        if (data) setShop(data);
      }
    }
    loadShop();
  ...
  // Real Dynamic Rates from Shop Settings (with fallbacks)
  const rates = {
    bwSingle: shop?.bw_single_rate || shop?.bw_rate || 2.0,
    bwDouble: shop?.bw_double_rate || 3.0,
    colorSingle: shop?.color_single_rate || shop?.color_rate || 5.0,
    colorDouble: shop?.color_double_rate || 8.0,
  };
  ...
  const triggerPayment = async () => {
    ...
    try {
      ...
      const activeShopId = shop?.id || 'shop-01';
      ...
      setTimeout(() => {
        ...
        if (qrRef.current && win.QRCode) {
          ...
          const upiTarget = shop?.upi_id || 'bhupeshdewangan444@okhdfcbank';
          const shopNameEncoded = encodeURIComponent(shop?.name || 'ScanToPrint Store');
          const upiString = `upi://pay?pa=${upiTarget}&pn=${shopNameEncoded}&am=${totalCost}&cu=INR`;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-extrabold text-base text-white leading-tight">
              {shop?.name || 'Balod Central Xerox & Cyber'}
            </h1>
...
export default function CustomerUploadPortal() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#090d16] flex items-center justify-center text-slate-400 text-xs">Loading Print Portal...</div>}>
